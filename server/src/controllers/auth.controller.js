...
export const phoneAuth = async (req, res) => {
  try {
    ...
    const decoded = await admin.auth().verifyIdToken(firebaseIdToken); // [web:162]
    ...
    // Find user BEFORE possible create
    ...
    res.status(500).json({ message: error.message || "Phone auth failed" });
  }
};